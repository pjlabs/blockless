package io.github.pjlabs.blockless.context.slf4j;
...
public final class Slf4jMdcContextPropagator implements ContextPropagator {
  ...
  /**
   * Restores the MDC snapshot from {@link #attach(Object)}. When {@code previous} is not a {@code
   * Map<String, String>} (including {@code null}), clears MDC so the worker thread does not leak
   * context; callers should only pass values returned from {@code attach} for a successful attach.
   */
  @Override
