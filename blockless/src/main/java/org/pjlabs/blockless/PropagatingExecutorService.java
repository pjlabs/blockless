 ...
 * <p>
 * Every task submitted through this executor is wrapped so that context is captured at submission
 * time, attached before execution, and restored after execution.
 */
public final class PropagatingExecutorService implements ExecutorService {
    ...
    private PropagatingExecutorService(ExecutorService delegate, List<ContextPropagator> propagators) {
     ...
     * Wraps an existing {@link ExecutorService} with context propagation.
     */
    public static ExecutorService wrap(ExecutorService delegate, List<ContextPropagator> propagators) {
    ...
    private <T> List<Callable<T>> wrapAll(Collection<? extends Callable<T>> tasks) {
        return tasks.stream()
                .map(task -> CallableContext.wrap(task, propagators))
                .toList();
    }
}
